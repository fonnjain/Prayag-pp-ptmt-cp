...
type ActivityApp = "ops-dashboard" | "production-planning" | "production-monitoring";
...
export function ActivityTracker({ app }: { app: ActivityApp }) {
  useEffect(() => {
    let disposed = false;
    ...
    const emitPage = () => {
      ...
      lastRoute = route;
      if (sessionId !== null) {
        void send("event", { sessionId, kind: "page", name: "page_view", route });
      }
    };
    ...
    const heartbeat = () => {
      ...
      const active = visible && Date.now() - lastInteraction < 120_000;
      void send("heartbeat", { sessionId, route: window.location.pathname, visible, active });
      emitPage();
    };

    const onVisibility = () => { markInteraction(); };
