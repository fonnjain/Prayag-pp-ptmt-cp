    ...
    <div className="space-y-6 max-w-[1200px] mx-auto pb-10">
      ...
      <div className="space-y-4">
        {recommendations.map((rec) => {
          ...
          return (
            <Card key={`${rec.priority}-${rec.code}`} className="border-border/60">
              <CardContent className="pt-4 pb-4">
                <div className="flex items-start gap-4">
                  <div className="flex-shrink-0 w-8 h-8 rounded-full bg-primary/10 text-primary flex items-center justify-center font-bold text-sm">{rec.priority}</div>
                ...
                {recommendations.map((rec) => {
                  ...
                  return (
                    <tr key={`row-${rec.priority}-${rec.code}`} className="border-b border-border/20 hover:bg-muted/20">
                      ...
                      <td className="py-1.5"><Badge variant="outline" className={`text-xs ${effort.className}`}>{rec.effort}</Badge></td>
                    </tr>
                  );
                })}
              </tbody>
