...
// Retry once on FAST if DEEP errors. Returns the model ACTUALLY used so callers
// can record accurate provenance: { resp, modelUsed, tierUsed, downgraded }.
async function callWithFallback(client, sel, payload) {
  try {
    const resp = await client.messages.create({ model: sel.model, max_tokens: sel.maxTokens, ...payload });
    return { resp, modelUsed: sel.model, tierUsed: sel.tier, downgraded: false };
  } catch (e) {
    if (sel.tier === 'deep') {
      const resp = await client.messages.create({ model: FAST, max_tokens: MAX_FAST, ...payload });
      return { resp, modelUsed: FAST, tierUsed: 'fast', downgraded: true };
    }
    throw e;
  }
}
