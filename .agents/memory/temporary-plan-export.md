...
**How to apply:** Add export routes against `plan_run_inputs`/`plan_run_results`; keep external machine scheduling as a separate period-based payload rather than implementing a local Pass 2 fitter. Treat live production columns as a separate persisted data contract, not an implicit export-time Sheet read.